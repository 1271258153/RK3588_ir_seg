					...
					struct v4l2_subdev *sd)
{
	...
	if (hw->lane_mode == LANE_MODE_FULL) {
		...
		if (dphy->phy_index % 3 == DPHY1) {	// 用 lane0/1
			val |= (GENMASK(sensor->lanes - 1, 0) <<
				CSI2_DPHY_CTRL_DATALANE_ENABLE_OFFSET_BIT);
		...
		if (dphy->phy_index % 3 == DPHY2) {	// 用 lane2/3
			val |= (GENMASK(sensor->lanes - 1, 0) <<
	...
	/*  配置 dual/split 通路。
		在 GRF 寄存器里决定：
		1. lane 是 FULL 还是 SPLIT；
		2. 数据送到哪个 CSI host（GRF_DPHY_CSIHOST2~5_SEL）；
		3. 对应打开 DATALANE_EN / CLKLANE_EN。
	*/
...
static int rockchip_csi2_dphy_hw_probe(struct platform_device *pdev)
{
	...
	/* 在设备树中 csi2_dphy0_hw 节点根据rockchip,grf(寄存器块)拿到对应的regmap句柄 */
	...
	/* 在此绑定函数指针，在 phy-rockchip-csi2-dphy.c 中调用 */
	...
	/* 把dphy_hw存到platform上，然后由platform_get_drvdata() 取回 */
