...
static int csi2_dphy_s_stream(struct v4l2_subdev *sd, int on)
{
	...
	if (on) {
		...
		/* RK3568/RV1106 在probe函数中绑定dphy_hw, RK3588在开流时由CIF传入参数动态绑定 */
					   ...
					   struct v4l2_async_subdev *asd)
{
	...
	/* 分配一个 csi2_sensor 记录，保存 lanes/mbus/subdev */
	...
	/* 在媒体图里创建链路：sensor(source) ──link──> dphy(sink) */
...
static int rockchip_csi2dphy_media_init(struct csi2_dphy *dphy)
{
	...
	/* 扫描dphy节点下的port/endpoint,为每个端点创建asd,并注册到notifier中,
	   再通过rockchip_csi2_dphy_fwnode_parse回调函数解析端点的bus_type和lane数,
	   并保存到sensor_async_subdev->mbus和sensor_async_subdev->lanes中
	*/
	...
	/* 相邻的subdev匹配成功，调用bound函数：
	   把 sensor 的 lane/总线信息登记进 dphy，并在媒体图里建立 sensor → dphy 的数据链路;
	   解绑调用unbind函数，清除 sensor 的 sd 指针
	*/
...
static int rockchip_csi2_dphy_probe(struct platform_device *pdev)
{
	...
	/* 获取 dc-phy(Samsung) 和 dphy(Innosilicon) */
	...
	if (csi2dphy->drv_data->chip_id == CHIP_ID_RK3568 ||
		...
		csi2dphy->csi_info.csi_num = 1;	// dphy需要的csi通道个数
		csi2dphy->csi_info.dphy_vendor[0] = PHY_VENDOR_INNO;// 表示是Samsung的dcphy还是Innosilicon的dphy
		rockchip_csi2_dphy_attach_hw(csi2dphy, 0, 0);
	} else {
		csi2dphy->csi_info.csi_num = 0;	// 如果是RK3588，先设 0，等 CIF 通知
	}
	sd = &csi2dphy->sd;
	...
	/* csi2_dphy_subdev_ops
		└─ video
			└─ csi2_dphy_s_stream
	        	├─ csi2_dphy_get_sensor_data_rate()  读 sensor 的 LINK_FREQ，算 data_rate_mbps
   				├─ csi2_dphy_update_sensor_mbus()    从 sensor 拿 mbus/lane 数
   				├─ csi2_dphy_update_config()         动态 attach hw、定 lane 模式（FULL/SPLIT）
  				├─  csi2_dphy_enable_clk()			 使能 PHY 时钟
				└─ csi2_dphy_s_stream_start()		 操作硬件 phy 开流
	*/
