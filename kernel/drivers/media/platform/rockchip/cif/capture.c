				...
				enum v4l2_buf_type buf_type)
{
	...
	q->drv_priv = stream;
	q->ops = &rkcif_vb2_ops;		// REQBUFS/STREAM_ON等IOCTL命令调用此ops，队列的“流程回调”
	q->mem_ops = hw_dev->mem_ops;	// vb2 核心和 CIF 驱动都会调用，管底层分配和 DMA 同步
...
void rkcif_stream_init(struct rkcif_device *dev, u32 id)
{
	...
	INIT_LIST_HEAD(&stream->buf_head);				// 主 buffer 链表
	INIT_LIST_HEAD(&stream->rx_buf_head);			// 接收 buffer 链表
	INIT_LIST_HEAD(&stream->rx_buf_head_vicap);		// 在线模式（vicap）的接收链表
	INIT_LIST_HEAD(&stream->rockit_buf_head);		// rockit 用的链表
	spin_lock_init(&stream->vbq_lock);
	spin_lock_init(&stream->fps_lock);
	stream->state = RKCIF_STATE_READY;
	init_waitqueue_head(&stream->wq_stopped);		// 关流时等待 DMA 停的等待队列
	...
	/* is_compact 决定数据在内存里的排列方式（10bit 数据打包成 16bit 还是压缩存储），
	   这会影响 DMA 的地址步长和格式。
	 */
	if (dev->inf_id == RKCIF_DVP) {
		...
		if (dev->chip_id >= CHIP_RV1126_CIF)
			stream->is_compact = true;		// MIPI 通道在高版本芯片上是 compact
		else
			stream->is_compact = false;
	}
	...
	atomic_set(&stream->buf_cnt, 0);
	stream->rx_buf_num = 0;
	init_completion(&stream->stop_complete);	// 关流完成信号量
				...
				int mipi_id)
{
	...
	if (!stream->is_line_wake_up) {
		/* 从 buf_head 取一个新 buffer 写进 DMA 地址寄存器 */
	...
	if (cif_dev->chip_id < CHIP_RK3588_CIF &&
		...
		stream->frame_idx++;
	if (!stream->is_line_wake_up)
		/* 填 timestamp/sequence-> rkcif_vb_done_tasklet() → vb2_buffer_done() 
		 * 把填满的帧交给用户空间 DQBUF
		*/
...
/* rkcif_irq_pingpong_v1()		// 每一帧图像采完，硬件触发中断，都会走到这里
 * 1. 检查 MIPI/LVDS、DVP 并行接口路径
 * 2. 读中断状态，清中断
 * 3. 处理各类错误位
 * 4. 处理"帧结束"(frame end)
*/
void rkcif_irq_pingpong_v1(struct rkcif_device *cif_dev)
{
	...
	if (mbus->type == V4L2_MBUS_CSI2_DPHY ||
		...
		/* 4. 处理"帧结束"(frame end)  */
		for (i = 0; i < RKCIF_MAX_STREAM_MIPI; i++) {
			/* 从 intstat 里解析是哪个 stream（ID0~3）的帧结束了 */
			...
			switch (mipi_id) {	// 记录这一帧是 FRM0 还是 FRM1 完成的
			case RKCIF_STREAM_MIPI_ID0:
			...
			if (stream->dma_en & RKCIF_DMAEN_BY_VICAP) {
				...
				if (is_update)
					rkcif_update_stream(cif_dev, stream, mipi_id);	// 换 buffer + 投递
			} else if (stream->dma_en & RKCIF_DMAEN_BY_ISP) {
				v4l2_dbg(4, rkcif_debug, &cif_dev->v4l2_dev,
					 "dma capture by isp, dma_en 0x%x\n",
					 stream->dma_en);
				rkcif_update_stream_toisp(cif_dev, stream, mipi_id);// 在线模式（数据直给 ISP）
			...
			if (cif_dev->chip_id >= CHIP_RV1106_CIF)
				rkcif_modify_frame_skip_config(stream);	// 应用硬件跳帧寄存器
			if (stream->is_change_toisp) {				// 从回读模式切到在线模式
				stream->is_change_toisp = false;
			...
			if (!(stream->cur_stream_mode & RKCIF_STREAM_MODE_TOISP) &&
			    stream->lack_buf_cnt == 2) {	// 连续缺 2 个 buffer → 停 DMA 防止写空
				stream->to_stop_dma = RKCIF_DMAEN_BY_VICAP;
				rkcif_stop_dma_capture(stream);
				v4l2_dbg(4, rkcif_debug, &cif_dev->v4l2_dev,
			...
			spin_unlock_irqrestore(&stream->vbq_lock, flags);
			if (stream->to_en_scale) {	// 挂起等待启动的 scale 通道现在启动
				stream->to_en_scale = false;
				rkcif_scale_start(stream->scale_vdev);
			}
			rkcif_detect_wake_up_mode_change(stream);	// wait_line 唤醒模式动态切换
			if (cif_dev->chip_id < CHIP_RK3588_CIF &&
		...
		/* 5. 处理 START（SOF）和 LINE 中断 */
		for (i = 0; i < RKCIF_MAX_STREAM_MIPI; i++) {
			if (intstat & CSI_START_INTSTAT(i)) {	// 帧起始（SOF）
				stream = &cif_dev->stream[i];
				if (i == 0) {
					if (!stream->cur_skip_frame)
						rkcif_deal_sof(cif_dev);	// 记 fs_timestamp、发 SOF 事件、frame_idx++、同步多机
				} else {
					spin_lock_irqsave(&stream->fps_lock, flags);
				...
				stream->is_in_vblank = false;
				// 如果此时要停流/要开 DMA，在这里执行 rkcif_stop_dma_capture() / rkcif_enable_dma_capture()
				spin_lock_irqsave(&stream->vbq_lock, flags);
				if (stream->stopping && stream->dma_en) {
					if (stream->dma_en & RKCIF_DMAEN_BY_VICAP)
						stream->to_stop_dma = RKCIF_DMAEN_BY_VICAP;
			...
			if (intstat & CSI_LINE_INTSTAT_V1(i)) {	// 行中断（wait_line 提前唤醒模式）
				stream = &cif_dev->stream[i];
				if (stream->is_line_inten) {
					...
					if (cif_dev->sditf[0] && (cif_dev->sditf[0]->mode.rdbk_mode == RKISP_VICAP_RDBK_AUTO ||
						...
						rkcif_line_wake_up(stream, stream->id);	// 提前唤醒：提前处理一帧
					rkcif_modify_line_int(stream, false);		// 关行中断
					stream->is_line_inten = false;
				}
				v4l2_dbg(3, rkcif_debug, &cif_dev->v4l2_dev,
