...
int rkcif_plat_init(struct rkcif_device *cif_dev, struct device_node *node, int inf_id)
{
	...
	INIT_WORK(&cif_dev->err_state_work.work, rkcif_err_print_work); // 错误状态打印
	INIT_WORK(&cif_dev->sensor_work.work, rkcif_set_sensor_stream); // sensor 快速开关流
	INIT_DELAYED_WORK(&cif_dev->work_deal_err, rkcif_deal_err_intr);// 延迟恢复中断使能
	...
	/* 硬件缩放通道（CIF 内部 scaler，8/16/32 倍缩小 raw 图） */
	if (cif_dev->chip_id == CHIP_RK3588_CIF ||
	    cif_dev->chip_id == CHIP_RV1106_CIF ||
	    cif_dev->chip_id == CHIP_RK3562_CIF) {
	...
	/* 把 CIF 的所有视频节点（/dev/videoX）真正注册到内核，并挂上异步子设备 notifier
	   链路匹配成功后sensor → dphy → csi → cif，触发complete函数，
	   建立媒体链路，注册 video 节点
	*/
...
static int rkcif_plat_probe(struct platform_device *pdev)
{
	...
	/* rkcif_plat_init(cif_dev, node, inf_id) —— CIF 逻辑层"总装配"
	 *   ├─ 绑定 pipe.open / pipe.close / pipe.set_stream、isr_hdl
	 *   ├─ rkcif_stream_init			初始化buffer
	 *   ├─ v4l2_device_register		注册v4l2_device
	 *   ├─ media_device_init			初始化media_device
	 *   └─ rkcif_register_platform_subdevs()  ← 注册 /dev/videoX + 异步 notifier
	 *        └─ cif_subdev_notifier()
	 *              └─ (全部匹配) subdev_notifier_complete()
	 * 链路匹配/建链方向：sensor → dphy → csi → cif
	 * 开流方向（stream on）：cif → csi → dphy → sensor
	 */
	...
	/* 在 bootloader 阶段就把 sensor 打开、抓好一帧放进一块保留内存，
	   内核起来后 CIF 驱动直接去这块内存里"白拿"这一帧，快速出预览。
	   在capture.c中，根据is_thunderboot标志位，决定是否要去保留内存里拿这一帧数据。
	*/
