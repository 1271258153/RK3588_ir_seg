...
static int rkcif_plat_hw_probe(struct platform_device *pdev)
{
	...
	/* 中断来时：执行rkcif_irq_handler
	 * rkcif_irq_handler()
	 *	 └─ cif_hw->cif_dev[i]->isr_hdl(irq, cif_hw->cif_dev[i]); // 执行 dev.c 的 rkcif_irq_handler
	 *		   └─ rkcif_irq_pingpong_v1() // → capture.c 的 pingpong_v1，每帧完成触发中断
	*/
