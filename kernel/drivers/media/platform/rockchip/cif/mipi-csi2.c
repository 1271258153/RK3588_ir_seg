...
/* 记录上游 sensor 是谁、记录下游占用情况，并防止同一个 pad 被多个实体同时使用 */
			   ...
			   const struct media_pad *remote, u32 flags)
{
	...
	/* 如果是 source pad （下游方向，CSI2 → CIF/ISP）*/
	if (local->flags & MEDIA_PAD_FL_SOURCE) {
		if (flags & MEDIA_LNK_FL_ENABLED) {
			if (csi2->sink_linked[local->index - 1]) { // pad0 是 sink,source pad 从1开始编号
				ret = -EBUSY;
				goto out;
			}
	...
	} else {	/* 如果是 sink pad（上游方向，sensor/DSI → CSI2） */
		if (flags & MEDIA_LNK_FL_ENABLED) {
			if (csi2->src_sd) {
				ret = -EBUSY;
				goto out;
...
static int csi2_media_init(struct v4l2_subdev *sd)
{
	...
	num_pads = csi2->match_data->num_pads;	// rk3588是12

	/* pad 0 为 sink pad,其余都是 source pad */
		    ...
		    struct v4l2_async_subdev *asd)
{
	...
	/* 在媒体图里创建链路：dphy(source) ──link──> csi(sink)
	   最后一个参数是0，先不启用，先建成"未使能"，flags=0;
	   再由 media_entity_setup_link() 中 flags 从 0→ENABLED，有变化 → 触发 link_setup
	*/
	...
	/* 与phy不同的是，phy是静态的，不需要在运行时切换上游，不需要知道数据给谁;
	   但是csi上游可能有多个 sensor/DSI 源都能接到同一个 CSI2，但同一时刻只能选一个 → 必须记 src_sd;
	   一个 CSI2 有 11 个 source pad，可分别连到不同 CIF/ISP 通道，存在抢占 → 必须记 sink_linked[]（哪个口被占）。
	   media_entity_setup_link() 会调用 csi2_link_setup()
	*/
...
static int csi2_probe(struct platform_device *pdev)
{
	...
	csi2->sd.entity.ops = &csi2_entity_ops;	// 由 media_entity_setup_link 调用
	...
	/* 异步通知相关
	   csi2_notifier
	   		└─ v4l2_async_subdev_notifier_register
	   				└─ csi2_notifier_bound()  (sensor 异步匹配成功)
  							└─ media_create_pad_link(sensor→csi2)
  							└─ media_entity_setup_link(link, ENABLED)   ← 真正触发
...
static int csi2_hw_probe(struct platform_device *pdev)
{
	...
	csi2_hw->base = devm_ioremap_resource(&pdev->dev, res); // 把物理地址映射成内核虚拟地址
	if (IS_ERR(csi2_hw->base)) { // 多个 MIPI CSI2 共用同一段寄存器空间时，直接映射
	...
	if (irq > 0) {
		...
		/* 申请中断，rk_csirx_irq1_handler 用来处理 MIPI CSI2 接收错误 */
	...
	if (irq > 0) {
		irq_set_status_flags(irq, IRQ_NOAUTOEN);
		/* rk_csirx_irq2_handler 处理 CSIHOST_ERR2（PHY 层错误）*/
		ret = devm_request_irq(&pdev->dev, irq,
	...
	/* 挂到 platform device 上，供 csi2_attach_hw() 取用 */
