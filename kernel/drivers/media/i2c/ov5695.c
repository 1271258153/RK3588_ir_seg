...
static const char * const ov5695_supply_names[] = {
	...
	"dovdd",	/* Digital I/O power */		/* 1.8v */
	"dvdd",		/* Digital core power */	/* 1.2v */
};
...
static const struct ov5695_mode supported_modes[] = {
	{
		.width = 2592,
		.height = 1944,
		.max_fps = {		/* fps = numerator / denominator */
			.numerator = 10000,
			.denominator = 300000,
		},
		.exp_def = 0x0450,			// 默认曝光
		.hts_def = 0x02e4 * 4,		// 行总像素时钟数
		.vts_def = 0x07e8,			// 帧总行数
			   ...
			   u32 *val)
{
	...
	u8 *data_be_p;
	__be32 data_be = 0;
	/* 以0x300a为例，在小端机默认存放是0x0a、0x30，转换成大端地址，
	   转换完成后在小端机上寄存器的值返回0x0a30，此时在小端机存放是0x30、0x0a,
	   用(u8 *)&reg_addr_be 读出来正好是 0x30、0x0a，正好是 sensor 要的顺序。
	 */
	...
	/* data_be 是 __be32（大端 4 字节），内存下标 0 是最高字节、下标 3 是最低字节。
	   sensor 返回数据是高位在前，为了让读到的第 1 个字节（最高字节）落到“正确的高位位置”，
	   接收起点要往后挪 4 - len 个字节。
	 */
	...
	/* 在小端机上，将接收到的字节比如0x00、0x00、0x56、0x95,
	   按大端读成数值，再转成 CPU 小端，0x00005695 = 0x5695
	*/
...
static int __ov5695_power_on(struct ov5695 *ov5695)
{
	...
	/* 3. 上电，不是软件控制avdd->dovdd->dvdd
	      而是摄像头的引脚直接接到了电源上，启动开发板就有电压
	 */
	...
	/* 5. 在发起第一次 SCCB(类似I2C总线) 访问之前，至少要等 8192 个 XVCLK 周期。 */
	/* 8192 cycles prior to first SCCB transaction */
	delay_us = ov5695_cal_delay(8192);		// 342us
	usleep_range(delay_us, delay_us * 2);	// 延时342~684us
